...
module.exports = {
    ...
    async execute(interaction) {
        try {
            ...
            if (typ === 'subscribe') {
                let lastprice = "0€";
                let insert = `INSERT INTO subs (title, username, alreadyseen, lastprice) VALUES ('` + artikel.value + `', '` + author2 + `', '` + false + `', '` + lastprice + `')`;
                ...
                db_pool.connect(async function (err, db_client, done) {
                    ...
                    await interaction.reply({content: "Du erhälst ab sofort eine DM, falls ein Produktname mit dem Inhalt '" + artikel.value + "' im Mindstar auftaucht. ✅", ephemeral:true})
                })
                    ...
                    db_client
                        ...
                        .then(res => {
                            for (let i = 0; i < res.rows.length; i++) {
                                ...
                                options.push({label: res.rows[i].title, value: res.rows[i].title},);
                            }
                        })
                        .catch(err => {
                            console.error(err);
                        ...
                        .finally(async () => {
                            ...
                            try {
                                ...
                                await interaction.reply({content:"Es sieht so aus, als hättest du noch keine Abos. 😔", ephemeral:true})
                            }
                            done();
                        });
                })
