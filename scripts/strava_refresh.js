        ...
        db_client.query(select, (err, res) => {
          ...
          if (res) {
            for (let row of res.rows) {
              if (row.strava_expires_at < 3601) {
                //Laeuft token in weniger als einer Stunde ab?
                const options = {
                  hostname: "www.strava.com",
                  path:
                    "/api/v3/oauth/token?client_id=68240&client_secret=4b4bfa0504aa728620139c88fcd58e731b506146&grant_type=refresh_token&refresh_token=" +
                  ...
                  https.get(options, (response) => {
                    ...
                    response.on("end", function () {
                      if ((result != null) | (result != "")) {
                        ...
                        let update =
                          "UPDATE users SET strava_access_token = $1, strava_expires_at = $2, strava_refresh_token = $3 WHERE strava_id = $4";
                        let update_values = [
                          result.access_token,
                          result.expires_in,
