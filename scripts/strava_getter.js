...
const refresh = 'DROP TABLE IF EXISTS strava; CREATE TABLE IF NOT EXISTS strava (title varchar, distance varchar, date date, athlete varchar, type varchar)';
                                ...
                                https.get(options, (response) => {
                                    ...
                                    response.on('end', function () {
                                        ...
                                        for (let i = 0; i < result.length; i++) {
                                            ...
                                            let insertRow = "INSERT INTO strava (title, distance, date, athlete, type) VALUES ($1, $2, $3, $4, $5)";
                                            let insertRow_values = [tour.name, tour.distance, tour.start_date_local, name, tour.type];

                                            db_client.query(insertRow, insertRow_values, (err, res) => {
                                                if (err) {
                                                    return console.error(err);
                                                }
                                                if (res) {
