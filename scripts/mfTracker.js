...
const refresh =
  "DROP TABLE IF EXISTS products; CREATE TABLE IF NOT EXISTS products (full_title varchar, link varchar)";
    ...
    try {
      ...
      db_pool.connect(function (err, db_client, done) {
        ...
        for (let item of feed.items) {
          let insert =
            `INSERT INTO products (full_title, link)
                                  VALUES ('` +
            item.title +
            `', '` +
            item.link +
